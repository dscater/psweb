...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            @if ($o_modulo::canMod('pymes', 'crear'))
                <div class="row clearfix">
                    <div class="col-lg-2 col-md-2 col-sm-4 col-xs-6 botones">
                        <a href="{{ route('pymes.create') }}" class="btn btn-primary"><i
                                class="material-icons">local_shipping</i><span>Nuevo pyme</span></a>
                    </div>
                </div>
            @endif
            <!-- Basic Examples -->
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <input type="text" name="url_lista" id="url_lista" value="{{ route('pymes.index') }}"
                                hidden>
                                    ...
                                    <tbody id="datos_lista">
                                        @foreach ($proveedores as $key => $proveedor)
                                            <tr>
                                                ...
                                                <td><img src="{{ asset('imgs/proveedores/' . $proveedor->logo_p) }}"
                                                        width="75" height="75"></td>
                                                <td>{{ $proveedor->nom_rep_p }} {{ $proveedor->apep_rep_p }}
                                                    {{ $proveedor->apem_rep_p }}</td>
                                                <td>{{ $proveedor->cel_rep_p }}</td>
                                                <td>
                                                    @if ($o_modulo::canMod('pymes', 'eliminar'))
                                                        <input type="text" name="eliminar" class="url_eliminar"
                                                            value="{{ route('pymes.destroy', $proveedor->id) }}"
                                                            hidden>
                                                        <a href="#" title="Eliminar" class="eliminar"><i
                                                                class="material-icons eliminar">delete</i>
                                                        </a>
                                                    @endif
                                                    @if ($o_modulo::canMod('pymes', 'editar'))
                                                        <a href="{{ route('pymes.edit', $proveedor->id) }}"
                                                            title="Editar"><i class="material-icons editar">edit</i>
                                                        </a>
                                                    @endif
                                                    {{-- <a href="{{ route('pymes.show',$proveedor->datos_id) }}"><i class="material-icons">visibility</i></a> --}}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
