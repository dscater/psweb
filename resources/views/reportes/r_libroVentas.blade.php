...
<body>
    <div id="encabezado">
        ...
        <h4 id="texto">LIBRO DE VENTAS</h4>
    </div>
    <table class="periodo">
        <tr>
    ...
    <table border="1">
        <thead>
            <tr>
                ...
                <th rowspan="2">NIT del comprador</th>
                <th rowspan="2">Nombre o razón social del comprador</th>
        ...
        @if(count($ventas)>0)
            ...
            @php
                $array_fecha = explode('-',$value->fecha_venta);
                $total_fac = $total_fac + $value->pago_total;
                $total_neto = $total_neto + $value->pago_total;
                $total_iva = $total_iva + number_format($value->pago_total * 0.13,2,'.',',');
            @endphp
            <tr>
                <td>{{ $array_fecha[2] }}</td>
                <td>{{ $array_fecha[1] }}</td>
                <td>{{ $array_fecha[0] }}</td>
                <td>{{ $value->cliente->nit_ci }}</td>
                <td>{{ $value->cliente->nom_cli }}</td>
                <td>{{ $value->num_factura }}</td>
                <td>{{ $empresa->nro_aut }}</td>
                <td>{{ $value->codigo }}</td>
                <td>{{ $value->pago_total }}</td>
                <td>0</td>
                <td>0</td>
                <td>{{ $value->pago_total }}</td>
                <td>{{ number_format($value->pago_total * 0.13,2,'.',',') }}</td>
            </tr>
            @endforeach
            <tr class="totales">
                <td colspan="8">
